...
module.exports = exports = function (proto) {
  function compare(orig, compareTo, options, cb) {
    ...
    if (typeof options === 'object') {
      ...
      if (!options.metric) {
        args = ['-metric', 'mse'].concat(args)
      }
      else {
        args = ['-metric', options.metric].concat(args)
      }
    ...
    proc.on('close', function (code) {
      ...
      if (options.metric !== 'AE') {
        ...
        equality = parseFloat(match[1]);
      }
      else {
        equality = parseFloat(stdout);
      }
